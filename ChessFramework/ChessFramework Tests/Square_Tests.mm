...
- (void)testThatDefaultSquareHasZeroValue {
    Square s;
    XCTAssertEquals(std::static_cast<int>(s), (int)0, @"Default initialized square should have zero as index");
}

- (void)testInvalidFileAndRankValuesThrowOutOfRangeException {
    XCTAssertThrows(Square s(8, 9), @"Both file and rank out of range. Should throw an exception");
    XCTAssertThrows(Square s(5, 9), @"File in range, rank out of range. Should throw an exception");
    XCTAssertThrows(Square s(9, 5), @"File out of range, rank in range. Should throw an exception");
    XCTAssertThrows(Square s(SHRT_MAX, SHRT_MAX), @"Both file and rank out of range, maximum. Should throw an exception");
}
...
- (void)testDoesNotThrowExceptionForFileAndRankValuesInRange {
    for (unsigned short file = 0; file < 8; file++) {
        for (unsigned short rank = 0; rank < 8; rank++) {
            XCTAssertNoThrow(Square s(file, rank), @"Both file and rank in range. Should not throw an exception");.
        }
    }
}
...
- (void)testInvalidIndexValuesThrowOutOfRangeException {
    XCTAssertThrows(Square s(64), @"Should throw out of range exception");
    XCTAssertThrows(Square s(SHRT_MAX), @"Should throw out of range exception");
}

- (void)testDoesNotThrowExceptionForIndexValuesInRange {
    for (unsigned short i = 0; i < 64; i++) {
        XCTAssertNoThrow(Square s(i), @"Should not throw an exception.");
    }
}
